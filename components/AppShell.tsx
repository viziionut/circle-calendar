...
function GroupsPage({ groups, activeGroupId, onSelect, onCreate, onJoin, onInvite }: { groups: Group[]; activeGroupId: string; onSelect: (id: string) => void; onCreate: () => void; onJoin: () => void; onInvite: (id: string) => void }) {
  return <div className="page"><section className="pageTitle pageTitleActions"><div><small>CERCURILE TALE</small><h1>Grupuri</h1><p>Poți crea un grup, poți intra cu un cod și poți invita alte persoane.</p></div><div><button className="secondary" onClick={onJoin}><UserPlus/> Intră cu un cod</button><button className="primary" onClick={onCreate}><Plus/> Grup nou</button></div></section><div className="groupCards">{groups.map(group => <article key={group.id} className={group.id === activeGroupId ? "selectedGroup" : ""}><div className="groupIcon"><Users/></div><small>{group.id === activeGroupId ? "GRUP ACTIV" : "GRUP"}</small><h3>{group.name}</h3><p>{group.description || "Fără descriere"}</p><div className="groupCode"><span>Cod</span><code>{group.invite_code}</code></div><div className="cardActions"><button className="secondary" onClick={() => onSelect(group.id)}>{group.id === activeGroupId ? <><Check/> Activ</> : "Selectează"}</button><button className="primary" onClick={() => onInvite(group.id)}><Send/> Invită</button></div></article>)}</div></div>;
}
